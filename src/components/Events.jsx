            ...
            {eventsData.length > 0 ? (
                eventsData.map((item, index) => (
                 <div key={index} className="flex flex-col lg:flex-row items-center justify-between lg:items-center lg:justify-center text-[14px] cursor-pointer rounded-lg p-4 lg:p-2 mb-4 lg:mb-0">
                    <p className="text-center lg:text-left mb-2 lg:mb-0 lg:mr-4 lg:w-1/4 whitespace-nowrap text-[13px] font-semibold text-gray-500">{item.date}</p>
                    ...
                    <p className="text-center lg:text-left whitespace-nowrap text-[13px] font-semibold text-gray-500">{item.time}</p>
                </div>
            ))
            ) : (
